const calculateExpectedAnnualRevenue = (
  activeRevenueAvg,
  activityFrequency,
  decayRate
) => {
...
function calculateHistoricalSpectrum(activityRevenueMap, albums = []) {
  ...
  if (album_av_values.length > 0) {
    ...
    const albumVar =
      album_av_values.reduce((s, v) => s + Math.pow(v - albumMean, 2), 0) /
      albumCount;
...
const calculateActivityFrequency = (releaseDates) => {
  ...
  const activityFrequency =
    releaseDates.length > 0 ? releaseDates.length / activeYearsCount : 1;
  return {
...
export const computeKPIs = (
  ...
  if (valuationData.expected_revenue_calculation_start_date) {
    expectedAnnualRevenueCalculationStartDate = new Date(
      valuationData.expected_revenue_calculation_start_date
    );
  }
  ...
  const albums =
    valuationData.PFV?.av_a?.metrics || valuationData.PFV?.sub_data || [];
  ...
  for (let i = 0; i <= currentIndex; i++) {
    ...
    revenueStreams.forEach((key) => {
      const value =
        data[key] !== undefined && !isNaN(data[key]) ? data[key] : 0;
      if (key === "apv_t") {
    ...
    if (isActivity) {
      ...
      if (
        activityRevenueMap[currentYear] &&
        activityRevenueMap[currentYear] > 0
      ) {
  ...
  const activityRevenueAvg =
    activityCount > 0 ? activityRevenueSum / activityCount : 0;
  ...
  const expectedRevenueSpectrum = calculateHistoricalSpectrum(
    activityRevenueMap,
    albums
  );
  ...
  if (totalMOV > 0) {
    ...
    normalizedDiversityIndex =
      maxDiversityIndex > 0 ? diversityIndex / maxDiversityIndex : 0;
  }

  const maxRevenue =
    expectedAnnualRevenue * (1 + expectedRevenueSpectrum.spectrum);
  const minRevenue =
    expectedAnnualRevenue * (1 - expectedRevenueSpectrum.spectrum) > 0
      ? expectedAnnualRevenue * (1 - expectedRevenueSpectrum.spectrum)
